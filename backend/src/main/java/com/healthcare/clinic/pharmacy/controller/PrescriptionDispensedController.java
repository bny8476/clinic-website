...
public class PrescriptionDispensedController {
    ...
    public ResponseEntity<List<PrescriptionDispensed>> getMyDispensedPrescriptions(@AuthenticationPrincipal UserPrincipal pharmacist) {
        Long pharmacistId = pharmacist != null ? pharmacist.getUserId() : null;
        return ResponseEntity.ok(dispensedRepository.findByPharmacistId(pharmacistId));
    }

    @PostMapping
    @AuditableAction(module = "PHARMACY", action = "DISPENSE", resourceType = "PrescriptionDispensed", sensitivityLevel = "HIGH")
    public ResponseEntity<PrescriptionDispensed> dispensePrescription(
            @RequestBody PrescriptionDispensed dispensed,
            @AuthenticationPrincipal UserPrincipal pharmacist) {
        Long pharmacistId = pharmacist != null ? pharmacist.getUserId() : null;
        dispensed.setPharmacistId(pharmacistId);
